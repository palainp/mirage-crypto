...
typedef struct fe { WORD v[LIMBS]; } fe;
typedef struct fe_loose { WORD v[LIMBS]; } fe_loose;
...
static void fe_mul_impl(fe_limb_t out[LIMBS],
                        const fe_limb_t in1[LIMBS],
                        const fe_limb_t in2[LIMBS]) {
  assert_fe_loose(in1);
  assert_fe_loose(in2);
...
CAMLprim value mc_25519_scalar_mult_base(value out, value hash)
{
  CAMLparam2(out, hash);
  ge_p3 A;
  ge_p3_0(&A);
...
CAMLprim value mc_25519_pub_ok(value key)
{
  CAMLparam1(key);
  int success = 0;
  ge_p3 B;
